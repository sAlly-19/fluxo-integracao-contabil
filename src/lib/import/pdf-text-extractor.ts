...
export async function extractPdfTextFromBuffer(buffer: ArrayBuffer, timeoutMs = PDF_READ_TIMEOUT): Promise<string> {
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      reject(new Error('Timeout na leitura do PDF - o arquivo pode ser muito grande ou complexo. Tente um arquivo menor ou em outro formato (OFX/CSV).'));
    }, timeoutMs);
    ...
    loadPdfJs()
      .then((pdfjs) => {
        ...
        const documentInput: PdfDocumentInput = {
          ...
          disableWorker: true,
          // Use streaming to avoid loading entire PDF at once
