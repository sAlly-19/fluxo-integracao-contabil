...
export type SystemPayment = {
  id: string;
  userId: string; // references SystemUser.id or email
  userEmail: string;
  userName: string;
  companyName?: string;
  amount: number;
  ...
  paidAt?: string; // YYYY-MM-DD or ISO string
  paymentLink?: string; // PIX Copy-Paste code
  phone?: string; // Client's phone
  autoSendEnabled?: boolean; // If automated email is enabled for this bill
  manualSentAt?: string; // If invoice has been sent manually
};
