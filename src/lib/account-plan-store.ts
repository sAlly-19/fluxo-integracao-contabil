...
const ACCOUNT_PLAN_STORAGE_KEY = "fluxoic-account-plan-entries-v1";
...
export async function saveAccountPlanEntries(companyId: string | number, entries: AccountPlanEntry[]) {
  ...
  try {
    ...
    await Promise.all(existing.map(e => {
      const targetId = e.docId || (e.id ? e.id.toString() : "");
      return targetId ? deleteAccountPlan(targetId) : Promise.resolve();
    }));
