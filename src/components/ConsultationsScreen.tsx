...
export function ConsultationsScreen({ company, kind }: { company: Company; kind: ConsultationKind }) {
  return (
    <PageShell className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button asChild type="button" variant="outline">
            <Link to={`/empresas/${company.document ? company.document.replace(/\D/g, "") : company.id}/importar-arquivos`}>
      ...
      <ConsultationTabs companyId={company.document ? company.document.replace(/\D/g, "") : company.id} activeKind={kind} />
